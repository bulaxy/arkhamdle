...
import type { TransformedCard, AppSettings, TransformedInvestigator } from '../types';
import { fetchCards, transformCards, transformInvestigators } from '../services/ArkhamDbService';
...
export const GameProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadData = async (forceRefresh = false) => {
    setIsLoading(true);
    try {
      setLoadingMessage('Loading cards data (This may take a moment for the first time, ~11MB)...');
      ...
      setLoadingMessage('Error loading data. Please try again or check your connection.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    const init = async () => {
      ...
      if (savedSettings) {
        setSettingsState({
          ...
          storyGuesserScrambleWords: savedSettings.storyGuesserScrambleWords ?? true,
          storyGuesserScrambleLetters: savedSettings.storyGuesserScrambleLetters ?? false,
  ...
  const filteredCards = cards.filter(card => {
    const passPack = settings.filteredPacks.length === 0 || !settings.filteredPacks.includes(card.pack_name);
  ...
  const filteredInvestigators = investigators.filter(inv => {
    const passPack = settings.filteredPacks.length === 0 || !settings.filteredPacks.includes(inv.pack_name);
    const isWeakness = inv.subtype_code === 'basicweakness' || inv.subtype_code === 'weakness';
    const passWeakness = settings.includeWeakness || !isWeakness;
    return passPack && passWeakness;
  });
  ...
  return (
    <GameContext.Provider value={{ 
      cards, packs, settings, setSettings, isLoading, loadingMessage, 
      refreshData: () => loadData(true), filteredCards,
      investigators, filteredInvestigators
    }}>
      {children}
